window.addEventListener("load", function () {
  ...
  [...streams].forEach((item) =>
    item.addEventListener("mouseenter", function (e) {
      ...
      const toolTipOnStream = `<div class="toolTip">
       Stream on ${dataStream}</div>`;
      document.body.insertAdjacentHTML("beforeend", toolTipOnStream);
      const cords = e.target.getBoundingClientRect();
      const { top, left, width, height } = cords;

      const tooltipHeight = toolTipOnStream.offsetHeight;
      const triangleHeight = 10;
      const toolTipWidth = toolTipOnStream.offsetWidth;
      toolTipOnStream.style.left = `${left}px`;

      toolTipOnStream.style.top = `${top - tooltipHeight - triangleHeight}px`;
    })
  );
